  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      const {
        ...
        height: pHeight,
        widht: pWidth
      } = el.parentNode.getBoundingClientRect(); // 获取目标元素的页面位置信息与尺寸大小
      const { height: cHeight, width: cWidth } = el.getBoundingClientRect(); // 获取气泡框的宽高
      // 设置气泡框的位置
      switch (reactiveProps.position) {
        case "top":
          site["left"] = left;
